...
//		System.out.println(sequence.getWidth()+" "+sequence.getHeight());
		...
		f.getLayerList().forEach(l->{
			...
			at.concatenate(AffineTransform.getScaleInstance(l.getZmx()*scaleX,l.getZmy()*scaleY));
			at.concatenate(AffineTransform.getTranslateInstance(-w/2.0,-h/2.0));
			AlphaComposite alpha=AlphaComposite.SrcOver.derive(l.getAlpha()/10000.0f*transparency);
			target.drawTexture(t,at,alpha);


			if(SharedConstants.debugPixelLinerBoxes)
			{
				double[] dx = {0, w, w, 0};
				double[] dy = {0, 0, h, h};
